...
func writeDebianWatch(dir, gopkg, debsrc string, hasRelease bool, repack bool) error {
	...
	// Two newlines intentional to have the version line stand-alone as the first
	// paragraph.
	...
	// team's historical convention in suffix naming
	if repack {
		log.Printf("Setting debian/watch to repack with suffix +ds1")
		fmt.Fprint(f, "Repack-Suffix: +ds1\n")
